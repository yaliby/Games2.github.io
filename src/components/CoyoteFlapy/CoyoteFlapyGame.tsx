...
interface BossShot { x: number; y: number; vx: number; vy: number; r: number; spark: number }
...
/* ══════════════════════════════════════════════════════════════
   ...
   ──────────────────────────────────────────────────────────────
   Phase 1  (0–9):   Gentle intro. Wide gap, slow speed.
   Phase 2  (10–24): Ramps up steadily. Pipes start moving at 12.
   Phase 3  (25–44): Noticeably harder. Gap tightens, speed rises.
   Phase 4  (45–49): Pre-boss tension. Near-max difficulty.
   Boss     (50+):   Speed freezes at comfortable level; boss fight.

   easeInOut cubic keeps early game very forgiving and
   late game truly challenging without feeling unfair.
══════════════════════════════════════════════════════════════ */
...
function getDiffProgress(score: number): number {
  // Full difficulty reached at score 45 (just before boss)
  return Math.min(1, score / 45);
}

function getSpeed(score: number): number {
  if (score >= BOSS_AT) return BASE_SPEED + (MAX_SPEED - BASE_SPEED) * 0.45; // boss: fixed comfortable speed
  const p = easeInOutCubic(getDiffProgress(score));
  // Speed range: 2.3 → 6.1 (stronger ramp before boss)
  return BASE_SPEED + (6.1 - BASE_SPEED) * p;
}

function getGap(score: number): number {
  if (score >= BOSS_AT) return MIN_GAP + 30; // boss: give player some breathing room
  const p = easeInOutCubic(getDiffProgress(score));
  // Gap range: 205 → 126 (tight near boss, still fair)
  return BASE_GAP - (BASE_GAP - 126) * p;
}

function getSpawnF(score: number): number {
  // Interval: 95 frames early → 66 frames near boss. Noticeably denser.
...
function drawShot(ctx: CanvasRenderingContext2D, s: BossShot, frames: number) {
  ...
  sg.addColorStop(0,"#ffffff"); sg.addColorStop(0.4,"#ffee44"); sg.addColorStop(1,"rgba(255,100,0,0)");
  ctx.fillStyle=sg;
  ...
  ctx.strokeStyle="#ffee44"; ctx.lineWidth=1.5; ctx.globalAlpha=0.45*sparkP;
...
export default function CoyoteFlapyGame() {
  ...
  useEffect(()=>{
    ...
    function launchGalBossNow(){
      ...
      setDisplayScore(w.score);
      // Warp straight to Gal boss flow.
        ...
        if (w.preBossFrames > 0) {
          ...
          if (w.preBossFrames <= 0 && w.preBossKind) {
            activateBoss(
              w,
              w.preBossKind,
              w.preBossKind === "gal" ? "☠ GAL SHAPIRO INCOMING! ☠" : "⚡ BOSS INCOMING! ⚡"
            );
          }
        }
        ...
        if(w.mode==="normal"){
          ...
          for(const p of w.pipes){
            ...
            if(!p.scored && p.x+PIPE_W < CX-HIT_R){
              ...
              if(!p.nearMissed && Math.min(topClear,botClear) <= NEAR_MISS_D){
                p.nearMissed=true;
                w.score+=1; setDisplayScore(w.score);
          ...
          for(const g of w.gems){
            ...
            if(dx*dx+dy*dy<(HIT_R+GEM_R)*(HIT_R+GEM_R)){
              g.taken=true;
              w.score+=GEM_VALUE; setDisplayScore(w.score);
          ...
          if(candidateBossKind && w.pipes.length===0){
            if (candidateBossKind === "gal") {
              // Show warning text before Gal fight starts.
              w.preBossKind = "gal";
              w.preBossFrames = 90;
        ...
        if(w.mode==="boss"&&w.boss.active){
          ...
          if(b.introF>0){
            ...
            if(Math.abs(b.y-b.targetY)<8){
              const minY = 80;
              const maxY = isGalBoss ? 300 : (H - GH - 70); // runner can also dive near the bottom.
          ...
          for(const s of b.shots){
            ...
            if(s.x<-40||s.y<-40||s.y>H+40) {
              ...
              w.score++; setDisplayScore(w.score);
              b.hp--;
              b.hitFlash=6;
              ...
              if(b.hp<=0){
                ...
                addFloat(
                  W/2,H/2-130,
                  isGalBoss ? "GAL SHAPIRO DEFEATED! ★" : "BOSS DEFEATED! ★",
                  "#ffd700",36,true
                );
          ...
          if(b.hp<=b.maxHp/2&&!b.phase2){
            ...
            triggerFlash(w.flash,255,50,0,0.40);
          }
        }
  ...
  const hint=
    phase==="ready"   ?"Space · ↑ · tap to begin  •  1: Gal Boss":
    phase==="playing" ?"Space · ↑ · tap to flap  •  P pause  •  1 Gal Boss":
